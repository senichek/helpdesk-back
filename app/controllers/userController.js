...
const userController = {
  ...
  login: async (req, res) => {
    ...
    if (!email || !password) {
      ...
      if (exists.length > 0 && (await bcrypt.compare(password, exists[0].dataValues.password))) {
        // Remove password and add JWT
        const logged = {...exists[0].dataValues, password: "", jwt: userController.generateJWT(exists[0].dataValues.id)};
  ...
  signup: async (req, res) => {
    ...
    if (!name || !email || !role || !password) {
        ...
        if (exists.length > 0) {
            ...
            // Remove password and add JWT
            const created = {...dbResponse.dataValues, password: "", jwt: userController.generateJWT(dbResponse.dataValues.id)};
